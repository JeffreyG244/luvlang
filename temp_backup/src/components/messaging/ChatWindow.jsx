...
const ChatWindow = ({ conversationId, otherUserId }) => {
    const { user } = useAuth();
      ...
      <CardHeader className="pb-4">
        <CardTitle className="flex items-center gap-3">
          <div className="relative">
            ...
            <Circle className={`absolute -bottom-1 -right-1 h-3 w-3 ${isUserOnline(otherUserId)
      ...
      <CardContent className="flex-1 flex flex-col p-0">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 max-h-96">
          {isLoading ? (<div className="animate-pulse space-y-4">
              {[1, 2, 3].map(i => (<div key={i} className="h-10 bg-gray-200 rounded max-w-xs"></div>))}
            </div>) : messages.length === 0 ? (<p className="text-gray-500 text-center">
              No messages yet. Start the conversation!
            </p>) : (messages.map((message) => (<div key={message.id} className={`flex ${message.sender_id === user?.id ? 'justify-end' : 'justify-start'}`}>
          ...
          <form onSubmit={handleSendMessage} className="space-y-2">
            <div className="flex gap-2">
              <div className="flex-1 relative">
                <Input value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Type a message..." className={`${isMessageTooLong ? 'border-red-500' : ''}`} disabled={isSending} maxLength={LIMITS.MESSAGE_MAX_LENGTH + 100} // Allow slight overflow for warning
    />
                {isMessageTooLong && (<AlertTriangle className="absolute right-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-red-500"/>)}
              </div>
              <Button type="submit" size="icon" disabled={isSending || !newMessage.trim() || isMessageTooLong}>
            ...
            {isMessageTooLong && (<p className="text-red-500 text-xs flex items-center gap-1">

...
const NavigationTabs = () => {
    ...
    return (<div className="w-full bg-white/80 backdrop-blur-sm border-b border-purple-200 mb-6 sticky top-0 z-40">
      <div className="container mx-auto px-4">
        <Tabs value={currentPath} className="w-full">
          <TabsList className="w-full justify-start overflow-x-auto bg-transparent h-16 p-2">
            {tabs.map(({ value, label, path, icon: Icon }) => (<Link key={value} to={path} className="flex-shrink-0">
                <TabsTrigger value={path} className="flex items-center gap-2 px-4 py-3 rounded-lg data-[state=active]:bg-purple-100 data-[state=active]:text-purple-700 data-[state=active]:shadow-sm hover:bg-purple-50 transition-all duration-200 whitespace-nowrap">

...
const DataSeeder = () => {
    ...
    const generateCompatibilityAnswers = (profiles) => {
        return profiles.map(profile => {
            ...
            if (profile.partner_preferences) {
                ...
                if (prefs.includes('men') && !prefs.includes('women'))
                    seeking = 'Men';
                else if (prefs.includes('women') && !prefs.includes('men'))
                    seeking = 'Women';
                else if (prefs.includes('regardless of gender') || prefs.includes('any gender') || prefs.includes('transcends gender'))
                    seeking = 'Everyone';
            }
            return {
    ...
    return (<Card className="border-blue-200">
      ...
      <CardContent className="space-y-6">
        {/* Progress Bar */}
        {isSeeding && (<div className="space-y-2">
        ...
        <div className="flex gap-4">
          <Button onClick={seedDatabase} disabled={isSeeding} className="bg-blue-600 hover:bg-blue-700 text-white flex-1">
            {isSeeding ? 'Seeding...' : 'Seed Test Data'}
          </Button>
          
          <Button onClick={clearTestData} disabled={isSeeding} variant="outline" className="border-red-300 text-red-600 hover:bg-red-50">
            Clear Test Data
          </Button>
        </div>

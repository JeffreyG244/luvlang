...
const SecurityMonitor = () => {
    ...
    const loadSecurityActivity = async () => {
        try {
            ...
            const recentThreats = dbLogs.filter(log => log.severity === 'high' || log.severity === 'critical' ||
                log.event_type.includes('block') ||
                log.event_type.includes('violation')).length;
            ...
            const formattedFallback = fallbackLogs.slice(-10).map((log) => ({
    ...
    const getMessageFromDetails = (details, eventType) => {
        if (details && typeof details === 'object' && details.message) {
            return details.message;
        }
        return eventType.replace(/_/g, ' ');
    };
    const getActivityType = (severity) => {
        switch (severity) {
            case 'critical':
            case 'high':
                return 'error';
    ...
    const handleRunSecurityScan = async () => {
        try {
            ...
            await SecurityAuditService.logSecurityEvent('manual_security_scan', 'User initiated manual security scan', 'low');
    ...
    return (<Card className="border-green-200">
      <CardHeader>
      ...
      <CardContent className="space-y-4">
        <SecurityStatus sessionValid={securityStatus.sessionValid} deviceTrusted={securityStatus.deviceTrusted}/>

        <SecurityMetrics totalScans={securityMetrics.totalScans} threatsBlocked={securityMetrics.threatsBlocked}/>

        <SecurityActivity recentActivity={recentActivity}/>

        <SecurityFooter lastSecurityCheck={securityStatus.lastSecurityCheck} onRunSecurityScan={handleRunSecurityScan}/>
      </CardContent>
    </Card>);
};

...
const EnhancedProfileCard = ({ profile, onLike, onPass, swipeDirection }) => {
    return (<Card className="shadow-2xl border-0 overflow-hidden bg-white/95 backdrop-blur-sm">
      {/* Photo Section */}
      <div className="relative h-96 bg-gradient-to-br from-love-primary/5 to-love-secondary/5">
        <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent"></div>
        
        <img src={profile.photo_urls[0]} alt={`${profile.firstName}`} className="w-full h-full object-cover" onError={(e) => {
        ...
        {/* Swipe Indicators */}
        {swipeDirection && (<motion.div initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} className={`absolute inset-0 flex items-center justify-center ${swipeDirection === 'like'
                ? 'bg-green-500/20'
                : 'bg-red-500/20'}`}>
            <div className={`text-6xl font-bold ${swipeDirection === 'like'
                ? 'text-green-500'
                : 'text-red-500'}`}>
              {swipeDirection === 'like' ? '💚' : '💔'}
            </div>

...
import { Shield, CheckCircle, AlertTriangle, Verified } from "lucide-react";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="border-green-200">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-3">
              {verificationChecks.map((check, index) => (<div key={index} className="flex items-center justify-between">
                  <span className="text-sm">{check.check}</span>
                  <div className="flex items-center gap-2">
                    <span className="text-xs">{check.score}%</span>
                    {check.status === "verified" ? (<CheckCircle className="h-4 w-4 text-green-500"/>) : (<AlertTriangle className="h-4 w-4 text-yellow-500"/>)}
                  </div>
                </div>))}
            </div>
          </CardContent>
        ...
        <Card className="border-green-200">
          ...
          <CardContent className="space-y-3">
            {safetyFeatures.map((feature, index) => (<div key={index} className="p-3 bg-green-50 rounded-lg">
        ...
        <Card className="border-green-200">
          ...
          <CardContent className="space-y-4">
            {authenticMatches.map((match, index) => (<div key={index} className="border border-green-200 rounded-lg p-3 bg-green-50">
                ...
                <div className="flex flex-wrap gap-1">
                  {match.badges.map((badge, i) => (<Badge key={i} variant="outline" className="text-xs border-green-300">
                      {badge}
                    </Badge>))}
                </div>
              </div>))}

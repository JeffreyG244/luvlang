...
const BasicProfileQuestions = ({ answers, onAnswerChange }) => {
    return (<div className="space-y-8">
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <Select value={answers.age || ''} onValueChange={(value) => onAnswerChange('age', value)}>
                <SelectTrigger className="mt-1">
                  <SelectValue placeholder="Select your age"/>
                </SelectTrigger>
                <SelectContent>
                  {Array.from({ length: 63 }, (_, i) => i + 18).map((age) => (<SelectItem key={age} value={age.toString()}>
                      {age} years old
                    </SelectItem>))}
                </SelectContent>
              </Select>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <Select value={answers.city || ''} onValueChange={(value) => onAnswerChange('city', value)}>
                <SelectTrigger className="mt-1">
            ...
            <div>
              ...
              <Select value={answers.state || ''} onValueChange={(value) => onAnswerChange('state', value)}>
                <SelectTrigger className="mt-1">
            ...
            <div>
              ...
              <Input type="text" value={answers.zipcode || ''} onChange={(e) => onAnswerChange('zipcode', e.target.value)} placeholder="12345" className="mt-1" maxLength={5} pattern="[0-9]{5}"/>
            </div>
          </div>

          <div>
            <Label className="text-sm font-medium">Gender Identity *</Label>
            <Select value={answers.gender || ''} onValueChange={(value) => onAnswerChange('gender', value)}>
              <SelectTrigger className="mt-1">
                <SelectValue placeholder="Select your gender identity"/>
              </SelectTrigger>
              <SelectContent>
                {GENDER_IDENTITIES.map((gender) => (<SelectItem key={gender} value={gender}>
                    {gender}
                  </SelectItem>))}
              </SelectContent>
            </Select>
          ...
          <div>
            ...
            <Select value={answers.height || ''} onValueChange={(value) => onAnswerChange('height', value)}>
              <SelectTrigger className="mt-1">
            ...
            return (<SelectItem key={totalInches} value={`${feet}'${inches}"`}>
                      {feet}'{inches}"
                    </SelectItem>);
        })}
              </SelectContent>
      ...
      <Card className="border-pink-200">
        ...
        <CardContent className="space-y-6">
          <div>
            ...
            <RadioGroup value={answers.sexual_orientation || ''} onValueChange={(value) => onAnswerChange('sexual_orientation', value)} className="space-y-3">
              {SEXUAL_ORIENTATIONS.map((orientation) => (<div key={orientation.value} className="flex items-start space-x-3 p-3 rounded-lg border hover:bg-gray-50">
          ...
          <div>
            ...
            <Select value={answers.interested_in || ''} onValueChange={(value) => onAnswerChange('interested_in', value)}>
              <SelectTrigger className="mt-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Input type="number" min="18" max="100" value={answers.age_preference_min || ''} onChange={(e) => onAnswerChange('age_preference_min', e.target.value)} placeholder="18" className="mt-1"/>
            </div>
            <div>
              <Label className="text-sm font-medium">Preferred Age Range - Max *</Label>
              <Input type="number" min="18" max="100" value={answers.age_preference_max || ''} onChange={(e) => onAnswerChange('age_preference_max', e.target.value)} placeholder="65" className="mt-1"/>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="border-purple-200">
        ...
        <CardContent>
          <RadioGroup value={answers.relationship_goals || ''} onValueChange={(value) => onAnswerChange('relationship_goals', value)} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {RELATIONSHIP_GOALS.map((goal) => (<div key={goal.value} className="flex items-center space-x-3 p-4 rounded-lg border hover:bg-gray-50">
      ...
      <Card className="border-green-200">
        ...
        <CardContent className="space-y-6">
          {LIFESTYLE_QUESTIONS.map((question) => (<div key={question.id}>
              <Label className="text-sm font-medium mb-3 block">{question.question}</Label>
              <RadioGroup value={answers[question.id] || ''} onValueChange={(value) => onAnswerChange(question.id, value)} className="flex flex-wrap gap-3">
                {question.options.map((option) => (<div key={option.value} className="flex items-center space-x-2 p-2 rounded border hover:bg-gray-50">
      ...
      <Card className="border-orange-200">
        ...
        <CardContent className="space-y-6">
          {PERSONALITY_TRAITS.map((trait) => (<div key={trait.id}>
              <Label className="text-sm font-medium mb-3 block">{trait.question}</Label>
              <RadioGroup value={answers[trait.id] || ''} onValueChange={(value) => onAnswerChange(trait.id, value)} className="space-y-2">
                {trait.options.map((option) => (<div key={option.value} className="flex items-center space-x-3 p-3 rounded border hover:bg-gray-50">
                    <RadioGroupItem value={option.value} id={`${trait.id}-${option.value}`}/>
                    <Label htmlFor={`${trait.id}-${option.value}`} className="text-sm cursor-pointer flex-1">
                      {option.label}
                    </Label>
                  </div>))}
              </RadioGroup>
      ...
      <Card className="border-indigo-200">
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <Textarea id="bio" value={answers.bio || ''} onChange={(e) => onAnswerChange('bio', e.target.value)} placeholder="Share your interests, what you're passionate about, what you're looking for in a partner... (minimum 150 characters)" className={`mt-1 min-h-[120px] ${(answers.bio || '').length < 150 ? 'border-red-300' : ''}`} maxLength={500}/>
          ...
          <div>
            ...
            <Input id="interests" value={answers.interests || ''} onChange={(e) => onAnswerChange('interests', e.target.value)} placeholder="e.g., hiking, cooking, photography, travel, music..." className="mt-1"/>
          </div>

          <div>
            <Label htmlFor="profession" className="text-sm font-medium">Profession</Label>
            <Input id="profession" value={answers.profession || ''} onChange={(e) => onAnswerChange('profession', e.target.value)} placeholder="What do you do for work?" className="mt-1"/>
          </div>

          <div>
            <Label htmlFor="education" className="text-sm font-medium">Education</Label>
            <Select value={answers.education || ''} onValueChange={(value) => onAnswerChange('education', value)}>
              <SelectTrigger className="mt-1">

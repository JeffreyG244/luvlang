      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            <Input value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Enter test message..." onKeyPress={(e) => e.key === 'Enter' && sendTestMessage()}/>
            <Button onClick={sendTestMessage} disabled={loading || !newMessage.trim()}>
        ...
        <div className="space-y-3">
          ...
          <Button onClick={testRealTimeConnection} variant="outline" className="w-full">
        ...
        <div className="space-y-3">
          ...
          {testMessages.length === 0 ? (<p className="text-gray-500 text-center py-4">No test messages yet</p>) : (<div className="space-y-2">
              {testMessages.map((msg) => (<div key={msg.id} className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <Badge className={msg.status === 'sent' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}>
                        {msg.status === 'sent' ? (<CheckCircle className="h-3 w-3 mr-1"/>) : (<AlertCircle className="h-3 w-3 mr-1"/>)}
                        {msg.status}
                      </Badge>
                    </div>
                  </div>

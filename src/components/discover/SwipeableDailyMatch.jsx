...
const SwipeableDailyMatch = ({ match, onSwipe }) => {
    ...
    const getPhotoUrl = () => {
        ...
        return fallbackImages[gender] || fallbackImages.default;
    };
    ...
    return (<motion.div initial={{ scale: 0.9, opacity: 0, rotateY: 10 }} animate={{ scale: 1, opacity: 1, rotateY: 0 }} exit={{
            scale: 0.8,
            opacity: 0,
            x: swipeDirection === 'like' ? 400 : swipeDirection === 'pass' ? -400 : 0,
            rotate: swipeDirection === 'like' ? 20 : swipeDirection === 'pass' ? -20 : 0,
            transition: { duration: 0.4, ease: "easeInOut" }
        }} transition={{ duration: 0.5, ease: "easeOut" }} drag="x" dragConstraints={{ left: -100, right: 100 }} dragElastic={0.3} onDragEnd={handleDragEnd} onDrag={(event, info) => {
            setDragX(info.offset.x);
            setIsDragging(Math.abs(info.offset.x) > 10);
        }} onDragStart={() => setIsDragging(true)} whileDrag={{
            rotate: dragX * 0.1,
            scale: 1.05,
            cursor: 'grabbing',
            zIndex: 10
      ...
      <Card className="border-purple-200 hover:border-purple-300 transition-all duration-300 overflow-hidden">
        <CardHeader className="pb-3">
          <div className="relative">
            <div className="w-full h-64 bg-gradient-to-br from-purple-100 to-pink-100 rounded-lg overflow-hidden">
              <img src={photoUrl} alt={firstName} className="w-full h-full object-cover" onError={(e) => {
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center justify-center gap-6 pt-4">
            <Button onClick={() => handleSwipe('pass')} disabled={isLoading} size="lg" variant="outline" className="w-14 h-14 rounded-full border-3 border-red-400/60 hover:border-red-500 hover:bg-red-50 transition-all duration-200 shadow-lg hover:shadow-xl hover:scale-110">
              <X className="h-6 w-6 text-red-500"/>
            </Button>
            
            <Button onClick={() => handleSwipe('like')} disabled={isLoading} size="lg" className="w-14 h-14 rounded-full bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white border-0 shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-110">
      ...
      {/* Swipe Indicators */}
      <SwipeIndicators dragX={dragX} isVisible={isDragging}/>
    </motion.div>);
};

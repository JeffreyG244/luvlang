...
const SystemIntegrationTest = () => {
    ...
    const updateResult = (name, status, message, details) => {
        setResults(prev => prev.map(result => result.name === name
    ...
    const initializeTests = () => {
        const tests = [
            ...
            { name: 'Database Profiles', status: 'pending' },
            { name: 'Daily Matches Schema', status: 'pending' },
            { name: 'Compatibility Answers', status: 'pending' },
            { name: 'N8N Webhook', status: 'pending' },
            { name: 'Profile-Webhook Function', status: 'pending' },
        ];
        setResults(tests);
    };
    ...
    const testDatabaseProfiles = async () => {
        try {
            ...
            if (!firstProfile.photo_urls)
                missingFields.push('photo_urls');
            if (!firstProfile.city)
                missingFields.push('city');
            if (!firstProfile.state)
    ...
    return (<Card className="border-blue-200">
      ...
      <CardContent className="space-y-6">
        <Button onClick={runAllTests} disabled={isRunning || !user} className="bg-blue-600 hover:bg-blue-700 text-white w-full">
          {isRunning ? (<>
        ...
        {results.length > 0 && (<div className="space-y-3">
            ...
            {results.map((result, index) => (<div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                <div className="flex items-center gap-3">
                  {getTestIcon(result.name)}
                  <span className="text-sm font-medium">{result.name}</span>
                </div>
                <div className="flex items-center gap-2">
                  {result.message && (<span className="text-xs text-gray-600 max-w-xs truncate">
                      {result.message}
                    </span>)}
        ...
        {results.some(r => r.status === 'error') && (<div className="p-3 bg-red-50 border border-red-200 rounded-lg">
            <h4 className="text-sm font-medium text-red-800 mb-2">Issues Found:</h4>
            <ul className="text-xs text-red-700 space-y-1">
              {results.filter(r => r.status === 'error').map((result, index) => (<li key={index}>• {result.name}: {result.message}</li>))}
            </ul>
          </div>)}

        {results.length > 0 && results.every(r => r.status === 'success') && (<div className="p-3 bg-green-50 border border-green-200 rounded-lg">
